  ...
  try{
    ...
    await razorpayInstance.orders.create(options,(error,order)=>{
      if(error){
        ...
        return res.json({ success:false, message:error.description || "Something went wrong" });
      }
...
//After payment success webhook to update user credits
...
const verifyRazorpay = async(req,res)=>{
  try{
    ...
    if(orderInfo.status == 'paid'){
      //if yes find transaction data
      const transactionData = await transactionModel.findById(orderInfo.receipt);//recipts is from options and we have transaction id in receipt

      if(transactionData.payment){
        return res.json({
          success:false,
          message:"Payment failed since already paid"
        })
      }
